...
//        POOL_ready_d <= POOL_ready;

//    assign POOL_ready_posedge = POOL_ready && !POOL_ready_d;
//    assign frame_start = (ram_ren == 1'b0) &&
//                         (clk_div_fft1 != clk_div && clk_div_fft1 == 1);
...
//            ram_raddr <= '0;
//        end else begin
//            ram_raddr <= cnt_l + cnt_h * 47 + cnt + cnt_n * 45;
...
//                ram_wen   <= 1'b0;
//            end
//            else if (POOL_ready_posedge && ram_waddr != 412) begin
//                ram_waddr <= cnt_ready;
//                ram_wen   <= 1'b1;
//            end
//            else if (ram_waddr == 412 && POOL_ready_posedge) begin
//                ram_waddr <= 413;
//                ram_wen   <= 1'b1;
//            end
//            else begin
...
// Pool read module for 2x2 pooling operation
module pool_read_p2 (
    // Clock and reset signals
    input  logic        clk_b,
    input  logic        rstb_n,
    ...
    always_ff @(posedge clk_b or negedge rstb_n) begin
        if (rstb_n == 1'b0) begin
            ...
            if (frame_start)
                ...
                if (cnt_ready != 413 && ram_raddr != 0)
                    cnt_ready <= cnt_ready + 1'b1;
                else if (cnt_ready == 413)
                    cnt_ready <= '0;
            end
        end
    end
    ...
    always_ff @(posedge clk_b or negedge rstb_n) begin
        if (rstb_n == 1'b0) begin
            ...
            if (clk_div_fft1 != clk_div && clk_div_fft1 == 1)
                ram_ren <= 1'b1;
            else if (ram_raddr == 1690) // Last address for 47x37 input with 2x2 pooling
                ram_ren <= 1'b0;
        end
    end
    ...
    always_ff @(posedge clk_b or negedge rstb_n) begin
        if (rstb_n == 1'b0) begin
            ...
            else if (POOL_ready == 1 && ram_waddr != 412 && ram_raddr != 0) begin
                ram_waddr   <= cnt_ready;
                ram_wen     <= 1'b1;
            end
            else if (ram_waddr == 412 && POOL_ready == 1) begin
                ram_waddr   <= 413;
                ram_wen     <= 1'b1;
            end
            else if (ram_waddr == 413) begin
                ram_waddr   <= '0;
                ram_wen     <= 1'b0;
            end
        end
    ...
    always_ff @(posedge clk_b or negedge rstb_n) begin
        if (rstb_n == 1'b0) begin
            ...
            if (ram_ren_fft2 == 1'b1) 
                POOL_start <= 1'b1;
            if (ram_ren_fft2 == 1'b0) 
                POOL_start <= 1'b0;
        end
    end
